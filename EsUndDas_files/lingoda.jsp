	...
	usi_analytics = {
		...
		usi_app.main = function () {
			try {
				...
				usi_app.is_checkout_page = usi_app.url.match("/account") != null;
				usi_app.is_confirmation_page = usi_app.url.match("CONFIRMATION") != null;
				usi_app.is_home_page_en = window.location.href === "https://www.lingoda.com/en/" || window.location.href.startsWith("https://www.lingoda.com/en/?");
				usi_app.is_home_page_fr = window.location.href === "https://www.lingoda.com/fr/" || window.location.href.startsWith("https://www.lingoda.com/fr/?");
				usi_app.is_pricing_page = usi_app.url.match("/pricing/") != null;
				...
				// Booleans
				usi_app.is_return_visitor = usi_app.check_if_return_visitor();
				...
				usi_app.is_suppressed = !usi_app.is_forced && (usi_app.url.match("lingoda.com") == null ||
				...
				usi_aff.monitor_affiliate_pixel(function(){
					if (typeof USI_orderID == "undefined") {
						usi_commons.load_script("//www.upsellit.com/active/lingoda_pixel.jsp");
					}
				});

				if (usi_commons.gup("utm_campaign") == "P-Search") {
					usi_cookies.set("usi_affiliate", "1");
				}

				if (typeof document.referrer === "string" && document.referrer === "") {
					usi_cookies.set("usi_direct_traffic", "1");
				}
		...
		usi_app.load = function () {
			try {
				...
				if (usi_app.is_force_group === '') {
					// 90% Eligible, 10% Control split
					usi_app.is_force_group = Math.random() < 0.90 ? '1' : '0';
				}

				if (usi_app.is_home_page_en || usi_app.is_home_page_fr) {
					usi_split_test.instantiate_callback("51559", function(group) {
						if (group === '0') {
							usi_commons.log("Control Group: " + group);
						} else if (group === '1') {
							var key = '';
							if(usi_app.url.match("/en") != null) {
								key = '_en';
							} else if (usi_app.url.match("/fr") != null) {
								key = '_fr';
							}
							usi_commons.load_view("6ltoBt1kz9YBPIyWd9PVcBu", "51557", usi_commons.device + key);
						}
					});
				}
			} catch(err) {
		...
		usi_app.session_data_callback = function() {
			...
			if (usi_app.country === "au" || usi_app.country === "ca" || usi_app.country === "hk" || usi_app.country === "nz" || usi_app.country === "sg" || usi_app.country === "gb" || usi_app.country === "fr") {
		...
		usi_app.monitor_for_analytics = function() {
			try {
				if (typeof(usi_app.last_url) === "undefined" || usi_app.last_url != location.href) {
					...
					usi_analytics.send_page_hit("VIEW", "9121");
				}
